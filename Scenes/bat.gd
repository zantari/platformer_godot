...
func get_target():
	...
	var player_exists = is_instance_valid(player)
	var dist_to_player = position.distance_to(player.position) if player_exists else 99999.0
	...
	# 3. Wybór celu
	if player_exists and dist_to_player < notice_radius:
		target = player
	else:
